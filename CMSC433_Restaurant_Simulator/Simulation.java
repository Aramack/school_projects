...
public class Simulation {
	...
	public static List<SimulationEvent> runSimulation(
		...
		//Set up tables
		//List of tables: 
		SeatingArea.Initialize();
		for(int itt = 0; itt < numTables; itt++){
			SeatingArea.addTable();
		}
		Server.initialize();

		// Start up machines
		Kitchen.init(machineCapacity);


		// Let cooks in
		OrderQueue.Initialize(); //Queue where customers place orders and cooks get orders from
		Cook.setTotalCustomers(numCustomers);
		Thread[] cooks = new Thread[numCooks];
		for(int itt = 0; itt < numCooks; itt++){
			cooks[itt] = new Thread(new Cook("Cook "+ (itt+1)));
			cooks[itt].start();
		}
		...
		// Shut down machines
		Kitchen.Burgers.shutDown();
		Kitchen.Coffees.shutDown();
		Kitchen.Fries.shutDown();
